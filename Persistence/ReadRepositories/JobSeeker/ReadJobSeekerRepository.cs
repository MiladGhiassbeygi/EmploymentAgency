...
namespace Persistence.ReadRepositories
{
    public class ReadJobSeekerRepository : BaseReadRepository<JobSeeker>, IReadJobSeekerRepository
    {
        ...
        public async Task<List<JobSeeker>> FilterByTerm(string term, long userId, CancellationToken cancellationToken = default)
        {
            ...
            return await base.GetWithFilterAsync(x => (x.DefinerId == userId) && (x.FirstName.Contains(term) || x.LastName.Contains(term) 
                                                                                  || x.LinkedinAddress.Contains(term) || x.Email.Contains(term)));
        }
    }
}
